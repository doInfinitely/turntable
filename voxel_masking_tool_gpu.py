# voxel_masking_tool_gpu.py
# GPU-ACCELERATED interactive tool to mask/unmask voxels by casting rays
# Runs on Lambda GPU, displays on local machine via X11 forwarding
#
# Controls:
...
def make_intrinsics(h, w, fov_y_deg=45.0, device=DEVICE):
...
def build_pose_from_eye(eye_np, target_np, device=DEVICE):
    """
...
def cast_ray_through_pixel(px, py, h, w, K, R, t, near=0.1, far=5.0, n_samples=256, device=DEVICE):
    """
...
def generate_rays_full(h, w, K, R, t, n_samples=64, near=0.1, far=5.0, device=DEVICE):
    """
...
def mask_voxels_along_ray(sigma, pts_world, scene_radius=1.5, mask_value=0.0):
    ...
    # Set voxels along ray to mask_value (GPU accelerated)
    with torch.no_grad():
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python voxel_masking_tool_gpu.py <recon_volume.npz>")
        sys.exit(1)
    ...
    print()
    print("Starting in VOLUMETRIC rendering mode (GPU accelerated)")
    ...
    print(f"Volume loaded to GPU: {sigma_t.element_size() * sigma_t.nelement() / 1e6:.1f} MB")
    ...
    pygame.display.set_caption(f"GPU Voxel Masking Tool - {npz_path.name}")
    ...
    # Rendering mode
    render_mode = "volume"  # Always volume mode for GPU
    ...
    volume_n_samples = 128  # High quality
    
    # Pre-allocate background (GPU)
    bg = torch.linspace(20, 50, img_res[0], device=DEVICE).view(-1, 1, 1)
    bg = bg.expand(img_res[0], img_res[1], 3) / 255.0
    
    running = True
    frame_count = 0
    
    print("Ready! Rendering on GPU...")
    ...
    while running:
        ...
        with torch.no_grad():
            ...
            img_t = rgb_img[0].permute(1, 2, 0).clamp(0, 1)  # [H,W,3]
            
            # Composite over background (on GPU)
            alpha = img_t.sum(dim=-1, keepdim=True).clamp(0, 1)  # [H,W,1]
            img_t = img_t * alpha + bg * (1 - alpha)
        ...
        # FPS and mode
        fps = clock.get_fps()
        mode_text = font.render(f"GPU | Radius: {orbit_radius:.1f} | FPS: {fps:.1f}", True, (0, 255, 100))
        ...
        # Voxel count
        if frame_count % 30 == 0:  # Update every 30 frames to avoid slowdown
        ...
        pygame.display.flip()
        clock.tick(60)  # Target 60 FPS on GPU
        frame_count += 1
